...
function getNodeState(node) {
  ...
  if (resonanceBurstTicks > 0) return STATE_RESONANT;
  if (node.unlockCost > 0 && node.si >= node.unlockCost * 1.8) return STATE_RESONANT;
  return STATE_ACTIVE;
}

function getNodeStatLevels(node) {
  const levels = { Flow: 0, Insight: 0, Harmony: 0, Fortitude: 0, Essence: 0 };
  ...
  if (bonuses.flowEfficiency) {
    levels.Flow = Math.max(levels.Flow, Math.min(1, bonuses.flowEfficiency * 4.5));
  }
  if (bonuses.projectionRatePercent) {
    levels.Insight = Math.max(levels.Insight, Math.min(1, bonuses.projectionRatePercent * 4));
  }
  if (bonuses.projectionEcho) {
    levels.Insight = Math.max(levels.Insight, Math.min(1, bonuses.projectionEcho * 5));
  }
  if (bonuses.projectionDurationTicks) {
    levels.Insight = Math.max(levels.Insight, Math.min(1, bonuses.projectionDurationTicks / 500));
  }
  if (bonuses.maxBridges) {
    levels.Insight = Math.max(levels.Insight, Math.min(1, bonuses.maxBridges * 0.4));
  }
  if (bonuses.harmonyPower) {
    levels.Harmony = Math.max(levels.Harmony, Math.min(1, bonuses.harmonyPower * 4));
  }
  if (bonuses.resonanceBurstPercent) {
    levels.Harmony = Math.max(levels.Harmony, Math.min(1, bonuses.resonanceBurstPercent * 3));
  }
  if (bonuses.fortitude) {
    levels.Fortitude = Math.max(levels.Fortitude, Math.min(1, bonuses.fortitude * 1.6));
  }
  if (bonuses.essencePercent) {
    levels.Essence = Math.max(levels.Essence, Math.min(1, bonuses.essencePercent * 9));
  }
  if (bonuses.essenceFlatPerCycle) {
    levels.Essence = Math.max(levels.Essence, Math.min(1, bonuses.essenceFlatPerCycle / 50));
  }
  return levels;
}

function topStats(levels, count) {
  return STAT_ORDER
...
function findBlockingUpstreamNode(nodeId) {
