...
// Large file by design; keeps all visual-layer/draw-order logic in one place.
...
function createNodeVisual(node) {
  ...
  container.on("pointerdown", (event) => tryStartNodeDrag(node.id, event));
...
function redrawNode(node) {
  ...
  const hovered = node.id === hoveredTargetNodeId;

  // Ripple rings — count and opacity increase by state.
  const rippleCount = state === STATE_RESONANT ? 3 : state === STATE_ACTIVE ? 2 : 2;
  ...
  for (let i = 0; i < rippleCount; i += 1) {
    ...
    visual.ripples.stroke({
      width: 0.6 - i * 0.08,
      color: meta.primaryStroke,
      alpha: rippleAlphaBase - i * 0.07
    });
  }
  ...
  // Dashed outer boundary ring only for locked.
  ...
  visual.arcTrack.stroke({
    width: 1.4,
    color: meta.primaryStroke,
    alpha: state === STATE_LOCKED ? 0.45 : 0.55
  });
  ...
  if (maxProgress > 0) {
    ...
    visual.arcFill.stroke({
      width: state === STATE_RESONANT ? 4 : 2.4,
      color: meta.arcColor,
      alpha: 1,
      cap: "round"
    });
  ...
  for (let i = 0; i < 8; i += 1) {
    ...
    visual.compassTicks.moveTo(x1, y1);
    visual.compassTicks.lineTo(x2, y2);
    visual.compassTicks.stroke({
      width: isCardinal ? 0.9 : 0.6,
      color: meta.primaryStroke,
      alpha: 0.75
    });
  }

  // Stat dots at pentagram positions (72° apart, starting at top).
  ...
  for (let i = 0; i < STAT_ORDER.length; i += 1) {
    ...
    if (state === STATE_LOCKED) {
      radius = 2.2;
      fill = 0xc8b898;
      strokeColor = 0x8b7a5a;
      strokeAlpha = 0.45;
    } else if (state === STATE_RESONANT) {
      radius = 2.8 + dotLevel * 1.6;
      fill = STAT_COLOR_HEX[stat];
      strokeColor = 0xc87a14;
      strokeAlpha = 0.95;
    } else {
      radius = 2.4 + dotLevel * 1.3;
      fill = STAT_COLOR_HEX[stat];
      strokeColor = 0x2a6a4a;
      strokeAlpha = 0.6;
    }
  ...
  // Stat radar wedges inside the orb.
  visual.wedges.clear();
  if (state !== STATE_LOCKED) {
    const wedgeAlpha = state === STATE_RESONANT ? 0.28 : 0.18;
    for (let i = 0; i < STAT_ORDER.length; i += 1) {
      const stat = STAT_ORDER[i];
      const level = state === STATE_RESONANT
        ? Math.max(0.7, levels[stat])
        : levels[stat];
  ...
  // Inner orb fill + dashed ring.
  visual.orb.clear();
  visual.orb.circle(0, 0, NODE_ORB_RADIUS);
  visual.orb.fill({ color: meta.orbFill, alpha: 0.95 });
  const orbStrokeWidth = selected ? 2.2 : hovered ? 1.8 : 1.2;
  visual.orb.stroke({
    width: orbStrokeWidth,
    color: selected ? 0x8b6914 : meta.primaryStroke,
    alpha: 0.9
  });
  ...
  // Burst rays (resonant) / lock glyph (locked).
  ...
  if (state === STATE_RESONANT) {
    ...
    for (let i = 0; i < 8; i += 1) {
      ...
      const x2 = Math.cos(angle) * outer;
      const y2 = Math.sin(angle) * outer;
      visual.burstRays.moveTo(x1, y1);
      visual.burstRays.lineTo(x2, y2);
      visual.burstRays.stroke({
        width: isCardinal ? 1.3 : 0.7,
        color: 0xc87a14,
        alpha: 0.8
      });
    }
  ...
  // Stat callouts (active state only, to the left).
